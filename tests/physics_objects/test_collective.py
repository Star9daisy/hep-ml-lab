...
def test_init():
    obj = Collective(branch="jet", index=(1, 3))

    assert obj.branch == "jet"
    assert obj.index == slice(1, 3)

    assert obj.name == "jet1:3"
    assert obj.config == {"branch": "jet", "index": (1, 3)}

    # other names
    assert Collective(branch="jet", index=(None, None)).name == "jet"
    assert Collective(branch="jet", index=(1, None)).name == "jet1:"
    assert Collective(branch="jet", index=(None, 3)).name == "jet:3"


def test_special_methods():
    obj = Collective(branch="jet", index=(1, 3))

    assert obj == Collective(branch="jet", index=(1, 3))
    assert str(obj) == "jet1:3"
    assert repr(obj) == "Collective(branch='jet', index=(1, 3))"
...
def test_is_collective(single_names, collective_names, nested_names):
    ...
    assert is_collective(Collective(branch="jet", index=(1, 3))) is True
