...
class Madgraph5:
    """Wrapper of Madgraph5 to generate colliding events.

    It mainly uses commands from MadEvent interactive console which is opened
    using command `launch -i` in Madgraph5 CLI, to launch runs and save results.
    ...
    `mg5_output` has the same structure as original Madgraph5 output. This
    modified structure will be converted back to the original one in the future
    to be compatible with those generated by Madgraph5.
    ...
    n_events: int
        The number of events to be generated, which overwrites the `nevents` in
        the settings.
    seed: int
        The random seed that is set in `run_card`, `pythia8_card` and
        `delphes_card`, which overwrites the `iseed` in the settings.
    tag: str
        The tag to be added, which overwrites the `run_tag` in the settings.
    n_events_per_subrun: int
        The number of events per subrun. The default is 100000. Events are
        generated by command `multi_run n_subruns`. The `n_events` parameter is
        the total number of events. The `n_subruns` is calculated then to set
        the number of events for each subrun.
    ...
    | n_events    | set nevents commands             |
    | seed        | set iseed commands (not exactly) |
    | tag         | set run_tag commands             |
    """
    ...
    def launch(
        ...
        # <output>/
        #     run_1/
        #         mg5_output/Events/run_01/tag_1_delphes_events.root
        #         run.log
        #         commands
        #         tag
        #         cards
        ...
        # Create files in the run directory
        ...
        # Copy cards to the run directory
        cards_dir = run_dir / "cards"
        cards_dir.mkdir()

        # Find the pythia8 card and delphes cards
        pythia8_card = None
        delphes_card = None
        ...
        # Modify the commands to change the related path relative to the
        # mg5_output directory
        ...
        while (status != "Done") or process.poll() is None:
            ...
            if last_status != status:
                if show_status and last_status != "":
                    print(last_status)
                status = last_status
            # Currently HML could not handle the failed status well
            if status == "Failed":
                stderr = (
    ...
    def remove(self, name: str) -> None:
        """Remove one run according to its name."""
    ...
    def clean(self) -> None:
        """Clean the output directory and remove itself."""
...
class MG5Run:
    ...
    It now only supports the output structure of HML, not the original one of
    Madgraph5.
