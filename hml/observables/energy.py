...
class Energy(Observable):
    ...
    def read(self, event):
        if (branch := self.physics_object.read(event)) is None:
            ...
            self._value = branch.P4().E()

        elif is_collective_physics_object(self.physics_object):
            self._value = [obj.P4().E() if obj is not None else nan for obj in branch]

        else:
            self._value = []
            for main in branch:
                sub_values = [sub.P4().E() if sub is not None else nan for sub in main]
